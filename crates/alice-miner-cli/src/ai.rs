...
mod tests {
    ...
    fn render_status_never_emits_terminal_control_sequences() {
        ...
        let s = AiStatus {
            ...
            // v0.6.8 merge: the AM-REL-005 readiness row is part of the frame now, so
            // the injection test must cover it too (it renders a fixed string, but the
            // row must still be counted — otherwise the line-count assertion below
            // would silently stop being a forged-row detector).
        ...
        // 10 = the 9 pre-merge rows + the AM-REL-005 readiness row.
        assert_eq!(out.lines().count(), 10, "exactly the rows render_status writes: {out}");
